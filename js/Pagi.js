...
function pageButtons(pages) {
    ...
    var startPage = Math.max(currentPage - totalPagesToShow, 1);
    var endPage = Math.min(currentPage + totalPagesToShow, pages);
...
wrapper.innerHTML = `<button value=${1} class="page btn btn-sm btn-info">&#171; First</button>`;

wrapper.innerHTML += `<button value=${currentPage - 1} class="page btn btn-sm btn-info">&#9665; Previous</button>`;
...
if (currentPage < pages) {
    wrapper.innerHTML += `<button value=${currentPage + 1} class="page btn btn-sm btn-info">Next &#9655;</button>`;
}
